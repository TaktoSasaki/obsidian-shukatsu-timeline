...
就活（就職活動）の **受付期間・締切・面接・発表日** を、企業ノートのプロパティから自動集計して
...
> application periods, deadlines, interviews, and result-announcement dates — on a horizontal
...
- 📅 **横軸タイムライン**：各企業の `受付開始 / ES締切 / Webテスト締切 / 次回日程 / 発表日` を1本の時間軸に並べて表示。「今日」ラインつき。
- 📊 **受付期間バー**：`受付開始` と `ES締切` を両方入れると、エントリー受付期間を横バーで表示（左端＝受付開始・右端＝ES締切）。
- 🔴🟠🔵🟢 **種別で色分け**：ES締切=赤 / Webテスト=橙 / 面接・予定=青 / 発表=緑（受付期間バー=赤の半透明）。
...
選考状況: 応募済        # 内定 / お祈り / 辞退 はアラート対象外になる
受付開始: 2026-07-10    # ▭ 受付開始（ES締切とセットで「受付期間バー」を表示）
ES締切: 2026-07-28     # 🔴 ES/エントリー締切
Webテスト締切: 2026-07-30 # 🟠 Webテスト/適性検査の締切
次回日程: 2026-08-06    # 🔵 面接/予定
次回内容: 一次面接      # 🔵 のラベルに使われる
発表日: 2026-08-13     # 🟢 合否発表
---
```
...
| `range` | （なし） | 表示範囲の一括指定。`10d`=10日 / `2w`=2週間 / `3m`=3ヶ月 / `1y`=1年。**最優先** |
| `months` | （なし） | 今日から何ヶ月先まで表示するか（`range` 未指定時） |
| `weeks` | （なし） | 今日から何週間先まで表示するか（`range`/`months` 未指定時） |
| `past` | `7` | 今日より過去をどこまで表示するか。日数、または `2w` のように単位付きも可 |
| `future` | `120` | 範囲を明示しないときの右端の上限日数（イベントに合わせて自動調整） |
| `alert` | `7` | 上部アラートバーで「直近」とみなす日数 |

範囲の優先度は **`range` > `months` > `weeks` > 自動（`future`が上限）**。範囲外の締切/面接/発表はタイムラインには描画されません（アラートと直近リストは全件対象）。
...
| プロパティ | 種別 | 色 |
|---|---|---|
| `受付開始` | 受付期間バーの起点（`ES締切` とセット） | ▭ 赤(半透明) |
| `ES締切` | ES/エントリー締切（旧 `締切` も互換で読む） | 🔴 赤 |
| `Webテスト締切` | Webテスト/適性検査の締切 | 🟠 橙 |
