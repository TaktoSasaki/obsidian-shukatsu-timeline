...
- 📅 **横軸タイムライン**：各企業の `受付開始 / 締切 / 次回日程 / 発表日` を1本の時間軸に並べて表示。「今日」ラインつき。
- 📊 **受付期間バー**：`受付開始` と `締切` を両方入れると、エントリー受付期間を横バーで表示（左端＝受付開始・右端＝締切）。
- 🔴🔵🟢 **種別で色分け**：締切=赤 / 面接・予定=青 / 発表=緑（受付期間バー=赤の半透明）。
...
受付開始: 2026-07-10    # ▭ 受付開始（締切とセットで「受付期間バー」を表示）
締切: 2026-07-25       # 🔴 締切（一般的に ES／エントリー締切）
...
| プロパティ | 種別 | 色 |
|---|---|---|
| `受付開始` | 受付期間バーの起点（`締切` とセットで表示） | ▭ 赤(半透明) |
| `締切` | 締切（一般的に ES／エントリー締切） | 🔴 赤 |
